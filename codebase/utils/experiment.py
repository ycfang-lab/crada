...
from codebase.models import resnetdsbn
from ..datasets.imagelist import MultipleDomainsDataset
from .transform import ResizeImage, GaussianBlur
...
def get_model(model_name, pretrain=True):
    if model_name in models.__dict__:
        # load self-defined models
        backbone = models.__dict__[model_name](pretrain)
    else:
    ...
    return backbone
def get_dsbn_model(model_name, pretrain=True, num_domains_bn = 2):
    if model_name in resnetdsbn.__dict__:
        backbone = resnetdsbn.__dict__[model_name](num_domains_bn, pretrain)
    else:
        backbone = None
    return backbone
...
def get_dataset(dataset_name, root, source, target, train_source_transform, val_transform, train_target_transform=None):
    ...
    if dataset_name in datasets.__dict__:
        ...
        def concat_dataset(tasks, start_idx, **kwargs):
            return MultipleDomainsDataset([dataset(task=task, **kwargs) for task in tasks], tasks, domain_ids=
                                          list(range(start_idx, start_idx + len(tasks))))
        train_source_dataset = concat_dataset(root=root, tasks=source, transform = train_source_transform, start_idx=0)
        train_target_dataset = concat_dataset(root=root, tasks=target, transform=train_target_transform, start_idx=len(source))
        val_dataset = concat_dataset(root=root, tasks=target, transform=val_transform, start_idx=len(source))
