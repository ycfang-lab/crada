...
def calculate(source_feature: torch.Tensor, target_feature: torch.Tensor,
              device: torch.device, progress=True, training_epochs=10):
    """
    Calculate the :math:`\mathcal{A}`-distance, which is a measure for distribution discrepency.

    The definition is :math:`dist_\mathcal{A} = 2 (1 - 2\epsilon)`, where :math:`\epsilon` is the test
    error of a classifier trained to discrimiate the source from the target

    Args:
        source_feature (torch.Tensor): feature from source domain in shape :math:`(minibatch, F)`
        target_feature (torch.Tensor): feature from target domain in shape :math:`(minibatch, F)`
        device (torch.device)
    ...
    linear_classifier = LinearClassifier(feature.shape[1]).to(device)
    optimizer = SGD(linear_classifier.parameters(), lr = 0.01)
    for epoch in range(training_epochs):
        ...
        if progress:
            print("epoch {} accuracy: {} A-distance: {}".format(epoch, meter.avg, a_distance))
        return a_distance
